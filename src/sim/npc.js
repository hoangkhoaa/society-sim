...
export const ROLE_WEALTH_EXPECTATION = {
    leader: 120,
    merchant: 100,
    scholar: 80,
    craftsman: 60,
...
// ── Role assignment ────────────────────────────────────────────────────────
function assignRole(idx, total, ratios) {
...
function generateDescription(npc) {
...
// ── Fertility by age ───────────────────────────────────────────────────────
function fertilityByAge(age, gender) {
    if (gender === 'male')
        return age < 20 ? 0 : age < 50 ? 0.8 : age < 65 ? 0.3 : 0;
    return age < 18 ? 0 : age < 35 ? 0.9 : age < 45 ? 0.5 : 0;
}
// ── Create NPC ─────────────────────────────────────────────────────────────
export function createNPC(idx, total, constitution) {
    ...
    constitution.value_priority.forEach((val, rank) => {
        ...
        for (const [dim, b] of Object.entries(bias)) {
            valueBias[dim] = (valueBias[dim] ?? 0) + b * weight;
        }
    });
    ...
    const trust_in = Object.fromEntries(['government', 'market', 'opposition', 'community', 'guard'].map(inst => [
        inst,
        {
    ...
    const appearance = {
        height: ['short', 'average', 'tall'][randInt(0, 2)],
        build: ['slim', 'average', 'sturdy'][randInt(0, 2)],
        hair: age < 40 ? ['black', 'brown'][randInt(0, 1)]
            : age < 60 ? ['brown', 'gray'][randInt(0, 1)]
                : ['gray', 'white'][randInt(0, 1)],
        skin: ['light', 'medium', 'dark'][randInt(0, 2)],
    };
    const npcPartial = {
        id: idx,
        name,
...
// ── Per-tick NPC update ────────────────────────────────────────────────────
export function tickNPC(npc, state) {
...
function decayNeeds(npc, state) {
    ...
    if (npc.action_state === 'working' && foodAvailable)
        npc.hunger -= 2.0;
    if (npc.action_state === 'resting')
        npc.exhaustion -= 3.0;
    if (npc.action_state === 'socializing') {
        npc.isolation -= 2.5;
        if (npc.strong_ties.length < 3)
            npc.isolation += 1.0;
    }
...
function updateGrievance(npc, state) {
    let delta = 0;
    if (npc.hunger > 60)
        delta += (npc.hunger - 60) * 0.08;
    const neighborAvg = avgWealth(npc.weak_ties, state.npcs);
    if (npc.wealth < neighborAvg)
        delta += (neighborAvg - npc.wealth) / 200;
    const recentBetrayal = npc.memory.find(m => m.type === 'trust_broken' && state.tick - m.tick < 720);
    if (recentBetrayal)
        delta += Math.abs(recentBetrayal.emotional_weight) * 0.05;
...
function updateWorldview(npc, state) {
    let dDelta = 0;
    if (npc.stress > npc.stress_threshold)
        dDelta += (npc.stress - npc.stress_threshold) * 0.08;
    ...
    if (influencers.length === 0)
        return;
    const dims = ['collectivism', 'authority_trust', 'risk_tolerance', 'time_preference'];
...
// ── Wealth tick ────────────────────────────────────────────────────────────
export function computeProductivity(npc, _state) {
...
function wealthTick(npc, state) {
    if (npc.action_state === 'fleeing' || npc.action_state === 'confront')
        return;
    const productivity = computeProductivity(npc, state);
    npc.wealth = clamp(npc.wealth + (productivity - 0.5) * 0.1, 0, 5000);
}
// ── Lifecycle checks ───────────────────────────────────────────────────────
function checkLifecycle(npc, state) {
...
export function updateTrust(npc, institutionId, eventType, magnitude = 1.0) {
