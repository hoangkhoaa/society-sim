...
// ── Setup Conversation ─────────────────────────────────────────────────────
export async function setupGreeting(config) {
    const greeting = await callAI(config, buildSetupSystem(), 'Greet The Architect and ask what kind of society they want to build. Be concise — no more than 3 sentences.');
    addHistory('assistant', greeting);
    return greeting;
}
export async function setupChat(userMessage, config) {
...
// ── In-game chat ───────────────────────────────────────────────────────────
export async function handlePlayerChat(userMessage, state, config) {
    addHistory('user', userMessage);
...
export async function generateNPCThought(npc, state, config) {
...
${langDirective()}`;
    const thought = await callAI(config, 'You write inner monologue for characters in a social simulation. Keep it short, raw, and true.', prompt);
    return thought.trim();
}
...
function buildWorldContext(state) {
    ...
    const topGrievance = Object.entries(state.npcs.reduce((acc, n) => {
        ...
        .slice(0, 3)
        .map(([role, g]) => {
        const count = state.npcs.filter(n => n.role === role).length;
