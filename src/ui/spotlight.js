...
export async function openSpotlight(npc, state, config) {
    ...
    thoughtEl.textContent = t('sp.thought_loading');
    ...
    try {
        ...
        thoughtEl.textContent = t('sp.thought_fail');
        thoughtEl.className = 'sp-thought';
        console.error('Thought generation failed:', e);
    }
}
function renderStatic(npc, _state) {
    ...
    const heightLabel = t(`app.${npc.appearance.height}`);
    const buildLabel = npc.appearance.build === 'average'
        ? t('app.average') : t(`app.${npc.appearance.build}`);
    ...
    <div class="sp-section">
      ...
      ${statBar(t('sp.stress'), npc.stress, '#e24b4b')}
      ${statBar(t('sp.happiness'), npc.happiness, '#5dcaa5')}
      ${statBar(t('sp.grievance'), npc.grievance, '#ef9f27')}
      ${statBar(t('sp.hunger'), npc.hunger, '#e24b4b')}
      ${statBar(t('sp.exhaustion'), npc.exhaustion, '#7f77dd')}
      ${statBar(t('sp.isolation'), npc.isolation, '#378add')}
    </div>

    <!-- Worldview bars -->
    <div class="sp-section">
      <div class="sp-section-title">${t('sp.worldview')}</div>
      ${worldviewBar(t('sp.collectivism'), npc.worldview.collectivism, '#7f77dd')}
      ${worldviewBar(t('sp.auth_trust'), npc.worldview.authority_trust, '#ef9f27')}
      ${worldviewBar(t('sp.risk_tolerance'), npc.worldview.risk_tolerance, '#5dcaa5')}
    </div>
    ...
    <div class="sp-section" style="color:#e24b4b">
      ✝ ${t('sp.deceased')} — ${npc.lifecycle.death_cause
...
function worldviewBar(label, value, color) {
