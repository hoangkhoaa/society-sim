...
btnStart.addEventListener('click', async () => {
    ...
    if (!key) {
        showError(t('onboarding.err_no_key'));
        return;
    }
    aiConfig = {
    ...
    btnStart.textContent = t('onboarding.connecting');
    ...
    try {
        ...
        showError(`${t('onboarding.err_conn')} ${e.message}`);
...
async function sendSetupMessage(text) {
    if (!text.trim())
        return;
    setupInput.value = '';
...
// ── Game start ─────────────────────────────────────────────────────────────
async function startGame(constitution) {
    addFeedRaw(t('topbar.init'), 'info', 1, 1);
    ...
    addFeedRaw(tf('topbar.constitution_set', {
        n: world.npcs.length,
...
function setStat(valueId, value, statId, warnAt, dangerAt) {
...
async function sendChatMessage() {
    ...
    if (!msg || !aiConfig || !world)
        return;
    chatInput.value = '';
    addFeedRaw(`🌐 "${msg}"`, 'player', world.year, world.day);
    const removeThinking = addFeedThinking(t('err.thinking'));
    try {
        ...
        removeThinking();
        addFeedRaw(`${t('err.generic')} ${e.message}`, 'critical', world?.year ?? 1, world?.day ?? 1);
    }
}
function injectEvent(response) {
    if (!world || !response.event)
        return;
